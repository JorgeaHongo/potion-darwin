...
sub parse_genome_files {
  ...
  my (%sequences, %id2tmp_id, %tmp_id2id, %translation);
  ...
  while (my $file = readdir ($nt_files_dir)) {
    ...
    while (my $gene_info = $file_content->next_seq()) {
      ...
      $sequences{$accession_number} = $sequence;
...
#      my $a = <STDIN>;
  ...
  return (\%sequences, \%id2tmp_id, \%tmp_id2id, \%translation);
}
...
sub parse_cluster_file {
  my ($hash_of_groups_ref, $sequences_ref, $translation_ref, $parameters) = @_;
  my %clusters;
...
#       need to call this function here because cluster changes for cluster_name
        if (!validate_relative_sequence_size (\%clusters, \%specie_tracker, $sequences_ref, $translation_ref, $parameters, $cluster_name)) {
...
#        print LOG ('Cluster ', $cluster, ' empty, removed from the analysis', "\n");
    ...
    if ($parameters->{behavior_about_paralogs} != 4 && !validate_relative_sequence_size (\%clusters, \%specie_tracker, $sequences_ref, $translation_ref, $parameters, $cluster)) {
      if ($parameters->{behavior_about_bad_clusters} == 2) {
...
sub validate_relative_sequence_size {
  my ($clusters_ref, $specie_tracker, $sequences_ref, $translation_ref, $parameters, $key) = @_;
  ...
  for (my $i = 0; $i < scalar(@genes); $i++) {
    ...
    if (($seq_low > length($sequences_ref->{$gene})) #sequence length smaller than cutoff
       ||($seq_up < length($sequences_ref->{$gene}))) { #or greater
...
#        my $rel_max = $parameters->{relative_max_sequence_size};
        my $seq_len = length($sequences_ref->{$gene});
...
#        print LOG ('Gene ', $genes[$i], ' removed for containing sequence greater/smaller than specified relative size (rel_min: ', $parameters->{relative_min_sequence_size},' rel_max: ', $parameters->{relative_max_sequence_size},') within the group (dispersion: ',$dispersion_measure,') (seq_len: ',length($sequences_ref->{$gene}),') (upper: ',$seq_up,') (lower: ',$seq_low,')).', "\n");
#        print 'Gene ', $genes[$i], ' removed for containing sequence greater/smaller than specified relative size (rel_min: ', $parameters->{relative_min_sequence_size},' rel_max: ', $parameters->{relative_max_sequence_size},') within the group (dispersion: ',$dispersion_measure,') (seq_len: ',length($sequences_ref->{$gene}),') (upper: ',$seq_up,') (lower: ',$seq_low,')).', "\n";
#        print 'Gene ', $genes[$i], ' removed for containing sequence greater/smaller than specified relative size (rel_min - ', $parameters->{relative_min_sequence_size},' rel_max - ', $parameters->{relative_max_sequence_size},') within the group (dispersion - ',$dispersion_measure,') (seq_len - ',length($sequences_ref->{$gene}),') (',$seq_up,') (',$seq_low,')).', "\n";
...
#      my $a = <STDIN>;
  ...
  if ($measure eq "median") {
    ...
    for (my $i = 0; $i <= $#genes; $i++) {
      $lengths[$i] = length($seq_ref->{$genes[$i]});
    }
...
sub trim_all_equal_sequences_groups {
  ...
  foreach my $key (keys %{$clusters_ref}) {
    ...
    my @genes = split (/\*\*/, $clusters_ref->{$key}{cluster});
    foreach my $gene (@genes) {
      if (defined $sequences_ref->{$gene} && $sequences_ref->{$gene} ne $sequences_ref->{$genes[0]}) {
        $all_equal = 0;
        last;
      }
    }
...
sub create_sequence_files {
  my ($clusters_ref, $sequences_ref, $translation_ref, $id2tmp_id_ref, $ortholog_group) = @_;
  ...
  for (my $i = 0; $i < @genes; $i++) {
    my $gene = $genes[$i];
    if ((length($sequences_ref->{$genes[$i]}) > $largest_gene[0] && defined $clusters_ref->{$$ortholog_group}{reference_gene} && $clusters_ref->{$$ortholog_group}{reference_gene} =~ /$gene/)
     || (length($sequences_ref->{$genes[$i]}) > $largest_gene[0] && !defined $clusters_ref->{$$ortholog_group}{reference_gene})) {
      $largest_gene[0] = length($sequences_ref->{$genes[$i]});
      $largest_gene[1] = $i;
    }
  }
  if ($largest_gene[1] > -1) {
  ...
  foreach my $gene (@genes) {
    print $fh_out_nt (">$id2tmp_id_ref->{$gene}\n$sequences_ref->{$gene}\n\n") if (defined $id2tmp_id_ref->{$gene} && defined $sequences_ref->{$gene});  # filling a file with the nucleotide sequences of the ortholog group, fasta format

    my $seq_aa_ungapped = $translation_ref->{$gene};  # same for aminoacid sequences, but filtering gaps
...
sub calculate_dn_ds {
  ...
  if (-s "$$ortholog_group.cluster.aa.fa.aln.nt.phy.trim.paml.model$model") {
    ...
    my $BEB = exists_BEB($fh_paml) if ($model == 2 || $model == 8);
    close ($fh_paml);
    if (defined $ln_value && ($model == 1 || $model == 7 || $BEB)) {
  ...
  if ((-s "$$ortholog_group.cluster.aa.fa.aln.nt.phy.trim") && (-s "$$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy.trim.final_tree")) {
    ...
    die ("Problem with $$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy.trim.final_tree\n");
  }
  return;
}
...
sub parse_dn_ds_results {
  my ($parameters, $clusters_ref, $tmp_id2id_ref, $translation_ref) = @_;
...
sub print_task_time {
  ...
  print $fh_time_write ('Time taken by f_tree: ', $f_tree_time, "\n");
  close ($fh_time_write);
  return;
...
sub manage_task_allocation {
  my ($pid_to_ortholog_group, $n_process, $phi_pvalues, $nss_pvalues, $maxchi2_pvalues, $parameters, $id_rec_to_process, $f_tree_to_process, $model8_to_process, $model7_to_process, $model2_to_process, $model1_to_process, $seq_type) = @_;
  ...
  if (exists $pid_to_ortholog_group->{$completed_pid}) {
    ...
    my $f_tree_file = "$parameters->{project_dir}/intermediate_files/" . $ortholog_group . '/' . "$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy.trim.final_tree";
